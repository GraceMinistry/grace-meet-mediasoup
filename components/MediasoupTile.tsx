...
const MediasoupTile = ({ stream, participantName, isLocal }: MediasoupTileProps) => {
  ...
  return (
    <div className={`relative aspect-video bg-[#1C1F2E] rounded-2xl overflow-hidden border-2 transition-all duration-300 ${
      isTalking ? "border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.3)]" : "border-white/5"
    }`}>
      ...
      {!hasVideo && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-gray-500 text-xs font-medium uppercase tracking-widest">Audio Only</p>
        </div>
      )}
      ...
      <div className="absolute bottom-3 left-3 right-3 flex justify-between items-center">
        <div className="bg-black/60 backdrop-blur-md px-3 py-1 rounded-lg border border-white/10 flex items-center gap-2">
          {isTalking ? <Mic size={14} className="text-blue-400" /> : <MicOff size={14} className="text-gray-400" />}
          <span className="text-xs font-medium">{participantName} {isLocal ? "(You)" : ""}</span>
        </div>
        
        {!hasVideo && <div className="p-1.5 bg-red-500/20 rounded-md border border-red-500/50">
